...
async function init(){
    ...
    await getBiggerOrSmollerCityName(true);
    await getBiggerOrSmollerCityName(false);
}
...
async function createFiles(){
    ...
    for (state of states){
        ...
        await fs.writeFile(`./states/${state.Sigla}.json`, JSON.stringify(stateCities, null, 2));
    }
}
...
async function getStatesWithMoreOrLessCities(more){
    ...
    for (state of states){
        const count = await getCities(state.Sigla);
...
async function getBiggerOrSmollerCityName(bigger){
    const states = JSON.parse(await fs.readFile("./files/Estados.json"));
    const list = [];
